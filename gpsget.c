...
/*
 * set_opt
 * - 功能: 配置串口参数（波特率、数据位、停止位、校验位、超时等）
 * - 参数:
 *     fd - 打开的串口文件描述符
 *     bSpeed - 波特率（如 9600, 115200 等）
 *     dBits - 数据位（7 或 8）
 *     parity - 校验类型 'N','E','O'
 *     stopBit - 停止位 (1 或 2)
 * - 返回: 0 成功，-1 失败
 * - 说明: 同时设置 VTIME/VMIN 用于 read 的阻塞行为
 */
...
/*
 * dmconverttodeg
 * - 功能: 将 GPS 度分格式（DDMM.MMMM）转换为十进制度（度）
 * - 参数: dm - 字符串形式的度分值
 * - 返回: 十进制度值（double），出错返回 0.0
 */
...
/*
 * convertUTCToBeijingTime
 * - 功能: 将 NMEA 的 hhmmss UTC 时间字符串转换为北京时间（+8）并拆成 hour/minute/second
 * - 参数: utc - 字符串（至少 6 字符，格式 hhmmss）
 *           time - 输出长度至少为 3 的字节数组
 */
...
/*
 * gps_getfrom_uart
 * - 功能: 循环从 NMEA 串口读取数据并解析 $GNGGA 语句，用于更新全局 gps_info_uart
 * - 参数: fd - 打开的 GPS 串口文件描述符
 * - 关键行为:
 *     - 调用 set_opt 配置串口
 *     - 读取串口数据并查找 `$GNGGA` 报文
 *     - 解析 UTC、纬度、经度、定位状态、卫星数、HDOP、海拔等字段并保存在 gps_info_uart
 *     - 在读取失败或连续读取太短数据时尝试重启串口
 * - 说明: 该函数为阻塞函数，通常在单独线程中运行
 */
...
/*
 * getGPS
 * - 功能: 旧的/备用的 GPS 获取入口（读取并解析多个 NMEA 句子），在项目中可能未被使用
 */
...
void* gps_Thread(void* arg) {
    // 伪造固定的北京天安门附近 GPS 数据
    gps_info_uart.lon = 116.407396;   // 经度
    gps_info_uart.lat = 39.904200;    // 纬度
    gps_info_uart.gaodu = 50;         // 高度 50米
    
    // 伪造时间：12:00:00
    gps_info_uart.bj_time[0] = 12; // 时
    gps_info_uart.bj_time[1] = 0;  // 分
    gps_info_uart.bj_time[2] = 0;  // 秒
